...
dieIfNoSuchFile "$d/source.txt.WORD.T.lc1.sl0.count" "$0,$LINENO: "
nb1=$(sumFreqEtat  "$d"/source.txt.WORD.T.lc0.sl0.count)
nb2=$(sumFreqEtat  "$d"/source.txt.WORD.T.lc1.sl0.count)
if [ $nb1 -ne $nb2 ]; then
